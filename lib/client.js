...
/* =========================================================================
    ...
    function createStageFeature() {
       ...
       * and the key both keep focus in the textarea).
       */
      ...
      function onKeyDown(event) {
        ...
        if (event.isComposing || event.keyCode === 229) return
        // Only act while the composer textarea is focused.
        if (!isComposerTextarea(event.target)) return
        var sessionId = sessionIdFromTarget(event.target)
        if (sessionId === null || sessionId === undefined) return
        event.preventDefault()
        event.stopImmediatePropagation()
        // Read the live draft from the input store via the DOM textarea value
        // fallback isn't needed: the button path uses useInput, but the key
        // path has no hook. Reuse the DOM value for the stage decision.
        var textarea = event.target
        var draft = textarea instanceof HTMLTextAreaElement ? textarea.value : ''
