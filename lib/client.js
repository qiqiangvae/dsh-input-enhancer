window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var FIREWORK_COUNT = 18

    function Firework(props) {
      var anchor = props.anchor // { x, y } viewport center of the lock button
      var angleStep = (Math.PI * 2) / FIREWORK_COUNT
      var particles = []
      for (var i = 0; i < FIREWORK_COUNT; i++) {
        var angle = angleStep * i
        var dist = 34 + (i % 4) * 9 // varied travel distance
        particles.push(React.createElement('span', {
          key: 'p' + i,
          className: 'dsh-firework-particle',
          style: {
            '--dsh-fw-color': FIREWORK_COLORS[i % FIREWORK_COLORS.length],
            '--dsh-fw-dx': (Math.cos(angle) * dist) + 'px',
            '--dsh-fw-dy': (Math.sin(angle) * dist) + 'px',
          },
        }))
      }
    ...
    function createLockFeature() {
      ...
      function LockButton(props) {
        ...
        if (bursting) {
          var rect = buttonEl ? buttonEl.getBoundingClientRect() : null
          var anchor = rect
            ? { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 }
            : { x: 0, y: 0 }
          return React.createElement(React.Fragment, null,
            button,
            React.createElement(Firework, { anchor: anchor }))
        }
        return button
      }
    ...
    function adoptStyles() {
      ...
      style.textContent = [
        ...
        '20%{opacity:1}',
        '100%{transform:translate(var(--dsh-fw-dx),var(--dsh-fw-dy)) scale(.3);opacity:0}',
        '}',
